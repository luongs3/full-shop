...
//    edit one product - admin
    public function edit($id = null)
    {
        ...
        if (isset($data['errors'])) {
            ...
            empty($product['option']) or $product['option-count'] = count($product['option']);
            return view("product.edit")->with("product", $product)->with("scripts", $scripts)->with('categories', $categories);
        }
    }
...
//        product option process
        if ($input['option-count'] > 0) {
            ...
            for ($i = 0; $i < $input['option-count']; $i++) {
                if(empty($input['value-'.$i])){
                    continue;
                }
                $input['option'][$i]['title'] = $input['title-'.$i];
                unset($input['title-'.$i]);
                $input['option'][$i]['title'] = $input['value-'.$i];
                unset($input['value-'.$i]);
            }
            unset($input['option-count']);
            $input['option'] = serialize($input['option']);
        }
//        file process
...
//    product detail
    public function productDetail($sku)
    {
        ...
        if(!empty($product['option'])){
            $product['option-count'] = count($product['option']);
            foreach ($product['option'] as $key => $option) {
                $product['option'][$key]['values'] = explode(',',$option['value']);
            }
        }
